from game_classes import Vector2, Entity, State_Machine, State
import settings as c
import sprites
import sounds
from etc import accelerate, clamp, get_flipped_sprite
import level
import pygame as pg
import random
...
class Mario(Entity):
    ...
    def update(self):
        if not self.freeze_input:
            if c.keys[pg.K_LEFT] and not c.keys[pg.K_RIGHT]:
                self.pressed_left = True
                c.ACCELERATION = -c.MARIO_ACCELERATION
            elif c.keys[pg.K_RIGHT] and not c.keys[pg.K_LEFT]:
                self.pressed_right = True
                c.ACCELERATION = c.MARIO_ACCELERATION
            else:
                c.ACCELERATION = 0

            if not c.keys[pg.K_LEFT]:
                self.pressed_left = False
            if not c.keys[pg.K_RIGHT]:
                self.pressed_right = False
    ...
    def state_events(self):
        if any(self.current_action_state == state for state in
               ['Move_State', 'Decel_State', 'Brake_State', 'Idle_State']):
