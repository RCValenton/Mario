...
from game_classes import Entity, State, State_Machine, Rectangle
import sprites
import sounds
...
class Goomba(Entity):
    ...
    def check_collisions(self, dx, dy):
        ...
        other_enemy = self.rect.check_collisions(
            [enemy for enemy in level.enemies if enemy is not self and enemy.is_active])
        ...
        if other_collider is not None:
            if dx > 0:
                ...
                self.vel.y = 0
        if hasattr(other_collider, 'state_machine') and any(
                other_collider.state_machine.get_state() == state for state in ['Bounce_State', 'Break_State']):
