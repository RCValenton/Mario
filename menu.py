...
class Menu:
    ...
    def input_actions(self):
        if c.keys[pg.K_UP] and not self.pressed_down and not self.pressed_up:
            self.selected += 1
            self.pressed_up = True
        if c.keys[pg.K_DOWN] and not self.pressed_up and not self.pressed_down:
            self.selected -= 1
            self.pressed_down = True

        if not c.keys[pg.K_UP]:
            self.pressed_up = False
        if not c.keys[pg.K_DOWN]:
            self.pressed_down = False
